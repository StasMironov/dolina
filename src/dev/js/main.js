...
// Order page
    ...
    if ($('.set__item').exists()) {
        $('.set__item').each(function (index) {
            $(this).on('click', function (event) {
                ...
                if (target.tagName !== "use" && target.tagName !== "svg" && !target.classList.contains('quantity') && !target.classList.contains('btn')) projectFunc.showCard('.set__items', index); return false;
            });
        });
    }
    ...
    if ($('.dish-slider').exists()) {
        ...
        gsap.set(
            [$('.dish__article')[0], $('.dish__weight')[0]], {
            y: 0,
            opacity: 1
        }
        );
                ...
                slideChangeTransitionEnd: function () {
                    ...
                    $('.dish__box').each(function (index) {
                        if (index != actIndex) {
                            ...
                            gsap.set(
                                [$('.dish__article')[index], $('.dish__weight')[index]], {
                                y: -30,
                                opacity: 0
                            }
                            );
